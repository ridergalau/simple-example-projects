...
public class MongoLocal {
	public static void main(String[] args) {
		...
		for (int x = 1; x <= 30; x++) {
			try {
				BasicDBObject doc = new BasicDBObject("_id", "peg_" + x).append("nama", "Isabela" + x)
						.append("alamat", "ciamis" + x).append("no_hp", "071596629192," + x)
						.append("email", "didon.rosll@gmail.com" + x).append("noRumah", "Blok no 989" + x);
				col.insert(doc);
